          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="display-heading mt-3 text-2xl">{track.domain}</h3>
              </div>
              <span
            ...
            <p className="mt-4 text-sm leading-7 text-[var(--color-copy)]">{track.why}</p>
            <p className="mt-4 text-sm text-[var(--color-muted)]">
              Featured: {track.featuredCourse}
            </p>
            <Link href={`/learn?domain=${encodeURIComponent(track.tag)}`} className="mt-5 inline-flex text-sm font-semibold text-[var(--color-heading)]">
              Explore track
            </Link>
          </article>
        ))}
